...
>
> **연구 질문 (RQ)**
> 1. 댓글로 주가를 맞힌 사람들 중에 **진짜 실력자가 있나, 아니면 다 운인가?**
> 2. (있다면) 그 **실력자를 감지할 수 있는가?**
>
> **접근 & 잠정 답**: RQ2를 "적중 댓글(C2/C3) 감지 분류기"로 조작화 — train1(신호 존재 확인) → train2(희소클래스 보강해 감지력↑). RQ1은 통계검정(단일글·사람지속성·뱃지·2차 held-out)으로 친다. **두 트랙이 같은 답으로 수렴**: 단일 댓글로는 적중이 운과 거의 구별되지 않고(분류기가 C2/C1을 못 가름 = 그 자체가 증거), 사람 단위 '실력'도 통제하면 운·편향으로 무너진다 → *이 데이터 기준* 실력은 거의 감지 불가.
...
- [~] **라벨링 — Fold A 단방향**: train_A_TSLA + test_A_NVDA (train_B/test_B는 후속)
  - [x] train_A_TSLA **12,635/20,206 (63%)** 라벨 완료·복구·정리 (분포: 리액션8823/예측3812, 방향 상승3126·하락655·없음8854)
  - [x] (해결됨) 고동시성 빈응답·재시도 과금으로 비용 초과 → 부분 데이터로 진행
  - [x] 버그 수정: 에러행 컬럼 불일치(CSV 깨짐) → 모든 행 동일 8컬럼(`OUTCOLS`)+견고한 읽기
  - [x] **라벨링 종료**: train_A **14,991** + test_A **3,667** (평가에 충분). 비용 초과로 조기 종료
...
- [x] **운 vs 실력 분석 — 핵심 발견(이층 구조)**:
  ...
  - **핵심 연결 (RQ1↔RQ2)**: "적중은 한 줄의 글만으로는 운과 구별이 어렵다" — 분류기가 C1(실패)/C2(적중)를 못 가르는 RQ2의 난점이 곧 RQ1의 증거(단일글엔 실력신호 희박)
  - 단서: 시장타이밍·방향편향 혼동 가능. fig11

### RQ1 — 운 vs 실력: 통계검정 트랙 (코드 `6_skill_check/`·`archive/`)
> 분류기(RQ2)와 **같은 질문을 사람·통계 각도**에서 친다 — 단일글·사람지속성·뱃지·2차 held-out 네 각도로 "실력이 운과 구별되나"를 검정. **네 각도 모두 통제 후 실력신호가 사라짐 → 이 데이터에선 운이 지배적**이라는 게 RQ1의 답. (분류기의 C1/C2 난점과 같은 결론으로 수렴)
...
| 결정 | 내용 | 이유 |
|---|---|---|
| **단방향(Fold A) + 부분 데이터** | TSLA학습 14,991 → NVDA평가 3,667. train_B/test_B·풀 데이터는 후속 | 라벨링 비용 초과(빈응답·재시도 과금, 총 $282) |
...
- `[~]` **양방향 교차(Fold B)** — 후속(train_B/test_B 라벨링 → Fold B 추가)
